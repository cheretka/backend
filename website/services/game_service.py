...
def game_types() -> Response:
    return jsonify({'game_types': list(this.game_server.game_factory.get_all_games())})
...
def check_if_game_exists(session_id: Optional[str] = None) -> Response:
    ...
    return json_game_exists_message("Game with given session id exists!", game_type) if result else json_game_exists_message("Game with given session id does not exist.", "", 404)
...
def check_if_name_in_game_exists(session_id: Optional[str], name: Optional[str]) -> Response:
    ...
    return json_player_name_exists_message(f"Player named '{name}' already exists!", 406) if result else json_player_name_exists_message(f"Player named '{name}' does not exists!")


def json_game_exists_message(message: str, game_type: str, status: int = 200):
    return jsonify({"message": message, "game_type": game_type}), status
